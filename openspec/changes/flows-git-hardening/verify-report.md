...
## Spec Compliance Matrix (41 scenarios across 5 capabilities)

| Capability | Scenario | Test | Result |
|---|---|---|---|
| flow-preflight | Accounts loaded | preflight.bats PF-2 | COMPLIANT |
| flow-preflight | Empty accounts array | preflight.bats PF-1 | COMPLIANT |
| flow-preflight | Flow aborts when preflight fails (relink) | (none — code at install.sh:98-99) | UNTESTED |
| flow-preflight | sd missing | preflight.bats PF-4 | COMPLIANT |
| flow-preflight | python3 missing | preflight.bats PF-6 | COMPLIANT |
| flow-preflight | rg missing | preflight.bats PF-5 | COMPLIANT |
| flow-preflight | All optional deps present | preflight.bats PF-7 | COMPLIANT |
| flow-preflight | Empty array with set -u | preflight.bats PF-1 + accounts-rerender WU-7.3 | PARTIAL |
| flow-preflight | Populated array iterates | gitconfig.bats GC-1 (implicit) | PARTIAL |
| gh-session | Successful gh switch (claude-acct) | gh-session.bats GH-1 | COMPLIANT |
| gh-session | gh not installed — non-fatal | gh-session.bats GH-3 | PARTIAL (warning text impl'd, assertion is return-code only) |
| gh-session | gh switch fails — non-fatal | gh-session.bats GH-4 | COMPLIANT |
| gh-session | Directory matches personal account | gh-session.bats GH-2 | COMPLIANT |
| gh-session | Directory matches no account | (none) | UNTESTED |
| gh-session | gentle-ai install fails — no persist | frameworks-hardening.bats EC-4 | COMPLIANT |
| gh-session | gentle-ai install succeeds — persist | frameworks-hardening.bats EC-4 success | COMPLIANT |
| gh-session | Update regenerates rendered zshrc | gh-session.bats GH-5/GH-7 | COMPLIANT |
| git-identity-fix | Dry-run default | fix-git.bats FG-1 | COMPLIANT |
| git-identity-fix | Apply flag proceeds | fix-git.bats FG-2 | PARTIAL — unreachable via CLI (C-2) |
| git-identity-fix | Confirm one / decline another | fix-git.bats FG-2 + FG-3 | COMPLIANT |
| git-identity-fix | No https remotes message | (none — message not implemented) | UNTESTED (W-2) |
| git-identity-fix | Repo under work PROJECTS_DIR | fix-git.bats FG-4 | COMPLIANT |
| git-identity-fix | Repo not under any PROJECTS_DIR | (none — code at lib/fix-git.sh:111) | UNTESTED |
| git-identity-fix | gh offer accepted | fix-git.bats FG-6 | COMPLIANT |
| git-identity-fix | gh offer declined | fix-git.bats FG-7 | COMPLIANT |
| git-identity-validation | Logged in and active | doctor-identity.bats GI-1/GI-3 | COMPLIANT |
| git-identity-validation | Logged in not active | doctor-identity.bats GI-4 | COMPLIANT |
| git-identity-validation | Not logged in | doctor-identity.bats GI-2 | COMPLIANT |
| git-identity-validation | gh not installed | (none — code at lib/doctor.sh:13-14) | UNTESTED |
| git-identity-validation | HTTPS remote detected | doctor-identity.bats GI-5 | COMPLIANT |
| git-identity-validation | SSH remote correct alias | doctor-identity.bats GI-6/GI-7 | COMPLIANT |
| git-identity-validation | No repos under PROJECTS_DIR | doctor-identity.bats GI-9 | PARTIAL (no-crash only; check_pass message not impl'd) |
| git-identity-validation | Alias/dir mismatch | doctor-identity.bats GI-8 | COMPLIANT |
| git-identity-validation | Alias matches directory | doctor-identity.bats GI-7 | COMPLIANT |
| gitconfig-preservation | Initial render empty file | gitconfig.bats GC-1 | COMPLIANT |
| gitconfig-preservation | Re-render preserves credential | gitconfig.bats GC-2/GC-3 | COMPLIANT |
| gitconfig-preservation | Re-render idempotent | gitconfig.bats GC-5 | COMPLIANT |
| gitconfig-preservation | Legacy backed up | gitconfig.bats GC-4 | COMPLIANT |
| gitconfig-preservation | Legacy external blocks preserved | gitconfig.bats GC-4 (indirect) | PARTIAL (preserved, but old WSK content duplicated — W-3) |
| gitconfig-preservation | Add account re-renders | accounts-rerender.bats GC-6/AR-3 | COMPLIANT |
| gitconfig-preservation | Edit account re-renders | accounts-rerender.bats AR-2 | COMPLIANT |

**Compliance summary**: 31/41 COMPLIANT, 5 PARTIAL, 5 UNTESTED. All 45 new capability tests pass at HEAD, BUT the change broke 18 previously-passing tests in adjacent doctor functionality (C-1).
...
|---|---|---|
| Gitconfig managed-section markers | Partial | Markers `# WSK:BEGIN/END` (spec) instead of design's `# >>> work-swift-kit >>>`; awk strip-and-reappend mirrored correctly; migration keeps old WSK content (W-3) |
| Flow preflight `require_state` | Deviated | Shipped `preflight_accounts`/`preflight_deps` instead; `rendered`/`linked` flags never implemented (W-6); doctor wiring wrong (C-1) |
| Doctor identity audit bounded | Yes | maxdepth-2 glob, exact-token gh parse, `git -C remote get-url origin` |
| fix-git dry-run + per-repo confirm + longest-prefix | Yes in lib | `_fix_git_resolve_acct` longest-prefix correct; CLI arg plumbing broken (C-2) |
| `_wsk_gh_switch` shared helper | Yes | Defined once, both wrappers call it, GLOBAL side-effect comment present, non-fatal with both spec warning messages |
| Dependency fallbacks (sd/python3) + EC-4 | Yes | EC-4/EC-6/EC-7 implemented and tested |
| No new `readlink -f` | Yes | Only pre-existing lib/stow.sh:42 |
| bash 3.2 / set -u safety | Yes | `${arr[@]+...}` and `${arr+x}` patterns used consistently in new code |
